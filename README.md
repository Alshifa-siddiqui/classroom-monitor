...
| | |
|---|---|
| ![Login](docs/screenshots/01-login.png) | ![Sign-up](docs/screenshots/02-signup.png) |
| **Sign-in** — role-based login | **Sign-up** — optional admin code for elevated accounts |
| ![Dashboard](docs/screenshots/03-dashboard.png) | ![Students](docs/screenshots/04-students.png) |
| **Dashboard** — session controls, live stats, charts | **Students** — auto-generated IDs (`STU-2026-NNN`) |
| ![Enrollment](docs/screenshots/05-enrollment.png) | ![Live overlay](docs/overlay_sample.png) |
| **Face enrollment** — multiple embeddings, quality-scored | **Live overlay** — name · ID · emotion · attention · confidence |
...
A multi-stage `Dockerfile` builds the React dashboard and serves it from the
FastAPI backend (one process, port 8000):

```bash
docker compose up --build
# open http://localhost:8000  (sign in admin / admin123)
```

SQLite is used by default (persisted in the `cm-data` volume); a PostgreSQL
service is included in `docker-compose.yml` (commented) — enable it with
`docker compose --profile postgres up --build`. The ~83 MB vision models are
**not** baked into the image (the app runs with detector `fallback` without
them); mount a populated `backend/models/` to enable live recognition, and note
that webcam access requires host-device passthrough.
